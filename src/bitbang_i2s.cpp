...
void bit_bang_get_sample(uint16_t *sample) {
    ...
    *sample = 0;
    ...
    digitalWrite(PIN_SCK, HIGH); // Set the WS low

    for (int bit = 15; bit >= -15; --bit) { // Loop through each bit of the sample
        digitalWrite(PIN_SCK, LOW); // Set the clock low
        //delayNanoseconds(SCK_LOW_NS); // Wait for half the clock period

        // Read the data bit
        if (bit >= 0) {
          int dataBit = digitalRead(PIN_SD);
    ...
    if (WS_SETUP_NS > 0) {
        delayNanoseconds(WS_SETUP_NS); // Setup time for the next word select, if needed
    }
}